...
describe("Interval Mining", function() {
  ...
  it("should mine a block on the interval", function(done) {
    ...
    web3.vns.getBlockNumber(function(err, number) {
      ...
      setTimeout(function() {
        // Get the first block (pre-condition)
        web3.vns.getBlockNumber(function(err, latest_number) {
    ...
    web3.vns.getBlockNumber(function(err, number) {
      ...
      web3.vns.sendTransaction({
        ...
        web3.vns.getTransactionReceipt(tx, function(err, receipt) {
          ...
          setTimeout(function() {
            // Get the first block (pre-condition)
            web3.vns.getTransactionReceipt(tx, function(err, new_receipt) {
  ...
  it("miner_stop should stop interval mining, and miner_start should start it again", function(done) {
    ...
    web3.currentProvider.send({
      ...
      // Get the first block (pre-condition)
      web3.vns.getBlockNumber(function(err, initial_number) {
        if (err) return done(err);

        // Wait .75 seconds (one and a half mining intervals) and ensure
        // the block number hasn't increased.
        setTimeout(function() {
          web3.vns.getBlockNumber(function(err, stopped_number) {
            if (err) return done(err);
            ...
            web3.currentProvider.send({
              ...
              setTimeout(function() {
                web3.vns.getBlockNumber(function(err, last_number) {
                  if (err) return done(err);
