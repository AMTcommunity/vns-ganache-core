...
describe("Transaction rejection", function() {
  ...
  before("lock account 1", function() {
    return web3.vns.personal.lockAccount(accounts[1])
  })
  ...
  function testTransactionForRejection(paramsOverride, messageRegex, done) {
    ...
    provider.send(request, (err, response) => {
      ...
      if (response.error) {
        ...
        web3.vns.getTransactionReceipt(response.result)
          .then((result) => {
        ...
        return done(new Error('vns_sendTransaction responded with empty RPC response'))
      }
    })
  }
})
