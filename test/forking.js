...
var Web3WsProvider = require('web3/packages/web3-providers-ws');
...
describe("Forking", function() {
  ...
  before("Gather forked accounts", function(done) {
    this.timeout(5000)
    forkedWeb3.vns.getAccounts(function(err, f) {
      if (err) return done(err);
      forkedAccounts = f;
      done();
    });
  ...
  before("Deploy initial contracts", function(done) {
    forkedWeb3.vns.sendTransaction({
      ...
      forkedWeb3.vns.getTransactionReceipt(tx, function(err, receipt) {
        if (err) return done(err);

        contractAddress = receipt.contractAddress;

        forkedWeb3.vns.getCode(contractAddress, function(err, code) {
          if (err) return done(err);
  ...
  before("Make a transaction on the forked chain that produces a log", function(done) {
    ...
    var forkedExample = new forkedWeb3.vns.Contract(JSON.parse(contract.abi), contractAddress);

    // TODO: ugly workaround - not sure why this is necessary.
    if (!forkedExample._requestManager.provider) {
      forkedExample._requestManager.setProvider(forkedWeb3.vns._provider);
    }
  ...
  before("Get initial balance and nonce", function(done) {
    async.parallel({
      balance: forkedWeb3.vns.getBalance.bind(forkedWeb3.vns, forkedAccounts[0]),
      nonce: forkedWeb3.vns.getTransactionCount.bind(forkedWeb3.vns, forkedAccounts[0])
    }, function(err, result) {
      if (err) return done(err);
  ...
  before("Set main web3 provider, forking from forked chain at this point", function(done) {
    ...
    forkedWeb3.vns.getBlockNumber(function(err, number) {
      if (err) return done(err);
  ...
  before("Gather main accounts", function(done) {
    this.timeout(5000)
    mainWeb3.vns.getAccounts(function(err, m) {
      if (err) return done(err);
      mainAccounts = m;
      done();
    });
  ...
  it("should fetch a contract from the forked provider via the main provider", function(done) {
    mainWeb3.vns.getCode(contractAddress, function(err, mainCode) {
      if (err) return done(err);
      ...
      forkedWeb3.vns.getCode(contractAddress, function(err, forkedCode) {
        if (err) return done(err);
  ...
  it("should be able to get the balance of an address in the forked provider via the main provider", function(done) {
    ...
    mainWeb3.vns.getBalance(first_forked_account, function(err, balance) {
      if (err) return done(err);
  ...
  it("should be able to get storage values on the forked provider via the main provider", function(done) {
    mainWeb3.vns.getStorageAt(contractAddress, contract.position_of_value, function(err, result) {
      if (err) return done(err);
  ...
  it("should be able to execute calls against a contract on the forked provider via the main provider", function(done) {
    var example = new mainWeb3.vns.Contract(JSON.parse(contract.abi), contractAddress);

    // TODO: ugly workaround - not sure why this is necessary.
    if (!example._requestManager.provider) {
      example._requestManager.setProvider(web3.vns._provider);
    }
  ...
  it("should ignore continued transactions on the forked blockchain by pegging the forked block number", function(done) {
    ...
    var example = new mainWeb3.vns.Contract(JSON.parse(contract.abi), secondContractAddress);
    ...
    var forkedExample = new forkedWeb3.vns.Contract(JSON.parse(contract.abi), secondContractAddress);
  ...
  it("should maintain a block number that includes new blocks PLUS the existing chain", function(done) {
    ...
    mainWeb3.vns.getBlockNumber(function(err, result) {
      if (err) return done(err);

      assert.equal(mainWeb3.utils.hexToNumber(result), 5);

      // Now lets get a block that exists on the forked chain.
      mainWeb3.vns.getBlock(0, function(err, mainBlock) {
        if (err) return done(err);

        // And compare it to the forked chain's block
        forkedWeb3.vns.getBlock(0, function(err, forkedBlock) {
          if (err) return done(err);
          ...
          mainWeb3.vns.getBlock(mainBlock.hash, function(err, mainBlockByHash) {
            if (err) return done(err);
  ...
  it("should have a genesis block whose parent is the last block from the forked provider", function(done) {
    forkedWeb3.vns.getBlock(forkBlockNumber, function(err, forkedBlock) {
      if (err) return done(err);

      var parentHash = forkedBlock.hash;

      var mainGenesisNumber = mainWeb3.utils.hexToNumber(forkBlockNumber) + 1;
      mainWeb3.vns.getBlock(mainGenesisNumber, function(err, mainGenesis) {
        if (err) return done(err);
  ...
  it("should represent the block number correctly in the Oracle contract (oracle.blockhash0), providing forked block hash and number", function() {
    ...
    return oracleContract = new mainWeb3.vns.Contract(JSON.parse(oracleOutput.interface))
      ...
      .then(function(oracle){
        ...
        if (!oracle._requestManager.provider) {
          oracle._requestManager.setProvider(mainWeb3.vns._provider);
        }
        return mainWeb3.vns.getBlock(0).then(function(block){
          return oracle.methods.blockhash0().call()
            .then(function(blockhash) {
              assert.equal(blockhash, block.hash);
              // Now check the block number.
              return mainWeb3.vns.getBlockNumber()
            })
  ...
  it("should return the correct nonce based on block number", function(done) {
    ...
    async.parallel({
      nonceBeforeFork: mainWeb3.vns.getTransactionCount.bind(mainWeb3.vns, forkedAccounts[0], forkBlockNumber - 1),
      nonceAtFork: mainWeb3.vns.getTransactionCount.bind(mainWeb3.vns, forkedAccounts[0], forkBlockNumber + 1),
      nonceLatestMain: mainWeb3.vns.getTransactionCount.bind(mainWeb3.vns, forkedAccounts[0], "latest"),
      nonceLatestFallback: forkedWeb3.vns.getTransactionCount.bind(forkedWeb3.vns, forkedAccounts[0], "latest")
  ...
  it("should return the correct balance based on block number", function(done) {
    ...
    // ensure we're pulling data off the correct provider in both cases.
    async.parallel({
      balanceBeforeFork: mainWeb3.vns.getBalance.bind(mainWeb3.vns, forkedAccounts[0], forkBlockNumber - 1),
      balanceAfterFork: mainWeb3.vns.getBalance.bind(mainWeb3.vns, forkedAccounts[0], forkBlockNumber + 1),
      balanceLatestMain: mainWeb3.vns.getBalance.bind(mainWeb3.vns, forkedAccounts[0], "latest"),
      balanceLatestFallback: forkedWeb3.vns.getBalance.bind(forkedWeb3.vns, forkedAccounts[0], "latest")
    }, function(err, results) {
      if (err) return done(err);
  ...
  it("should return the correct code based on block number", function(done) {
    ...
    async.parallel({
      codeEarliest: mainWeb3.vns.getCode.bind(mainWeb3.vns, contractAddress, "earliest"),
      codeAfterFork: mainWeb3.vns.getCode.bind(mainWeb3.vns, contractAddress, forkBlockNumber + 1),
      codeLatest: mainWeb3.vns.getCode.bind(mainWeb3.vns, contractAddress, "latest")
    }, function(err, results) {
      if (err) return done(err);
  ...
  it("should return transactions for blocks requested before the fork", function(done) {
    forkedWeb3.vns.getTransactionReceipt(initialDeployTransactionHash, function(err, receipt) {
      if (err) return done(err);

      forkedWeb3.vns.getBlock(receipt.blockNumber, true, function(err, referenceBlock) {
        if (err) return done(err);

        mainWeb3.vns.getBlock(receipt.blockNumber, true, function(err, forkedBlock) {
          if (err) return done(err);
  ...
  it("should return a transaction for transactions made before the fork", function(done) {
    forkedWeb3.vns.getTransaction(initialDeployTransactionHash, function(err, referenceTransaction) {
      if (err) return done(err);

      mainWeb3.vns.getTransaction(initialDeployTransactionHash, function(err, forkedTransaction) {
        if (err) return done(err);
  ...
  it("should return a transaction receipt for transactions made before the fork", function(done) {
    forkedWeb3.vns.getTransactionReceipt(initialDeployTransactionHash, function(err, referenceReceipt) {
      if (err) return done(err);
      assert.deepEqual(referenceReceipt.transactionHash, initialDeployTransactionHash)

      mainWeb3.vns.getTransactionReceipt(initialDeployTransactionHash, function(err, forkedReceipt) {
        if (err) return done(err);
  ...
  it("should return the same network version as the chain it forked from", function(done) {
    forkedWeb3.vns.net.getId(function(err, forkedNetwork) {
      if (err) return done(err);

      mainWeb3.vns.net.getId(function(err, mainNetwork) {
        if (err) return done(err);
