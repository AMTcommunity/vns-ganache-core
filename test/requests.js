...
var tests = function(web3) {
  ...
  before('create and fund personal account', function() {
    return web3.vns.getAccounts()
      .then(function(accs) {
        ...
        return web3.vns.personal.newAccount("password")
      }).then(function(acct) {
        personalAccount = acct
      })
  });

  describe("vns_accounts", function() {
  ...
  describe("vns_blockNumber", function() {
    it("should return initial block number of zero", function(done) {
      var number = web3.vns.getBlockNumber(function(err, result) {
  ...
  describe("vns_coinbase", function() {
    it("should return correct address", function(done) {
      web3.vns.getCoinbase(function(err, coinbase) {
        if (err) return done(err);
  ...
  describe("vns_mining", function() {
    it("should return true", function(done) {
      web3.vns.isMining(function(err, result) {
        if (err) return done(err);
  ...
  describe("vns_hashrate", function() {
    it("should return hashrate of zero", function(done) {
      web3.vns.getHashrate(function(err, result) {
        if (err) return done(err);
  ...
  describe("vns_gasPrice", function() {
    it("should return gas price of 0.02 szabo", function(done) {
      web3.vns.getGasPrice(function(err, result) {
        if (err) return done(err);
  ...
  describe("vns_getBalance", function() {
    it("should return initial balance", function(done) {
      web3.vns.getBalance(accounts[0], function(err, result) {
        if (err) return done(err);
    ...
    it("should return 0 for non-existent account", function(done) {
      web3.vns.getBalance("0x1234567890123456789012345678901234567890", function(err, result) {
        if (err) return done(err);
  ...
  describe("vns_getBlockByNumber", function() {
    it("should return block given the block number", function(done) {
      web3.vns.getBlock(0, true, function(err, block) {
        if (err) return done(err);
    ...
    it("should return null given a future block number", function(done) {
      web3.vns.getBlock(10000, true, function(err, block) {
        if (err) return done(err);
    ...
    it("should return transactions in the block as well", function(done) {
      web3.vns.sendTransaction({
        ...
        web3.vns.getBlock("latest", true, function(err, block) {
          if (err) return done(err);
          ...
          web3.vns.getBlock("latest", false, function(err, block) {
            if (err) return done(err);
  ...
  // Relies on the validity of vns_getBlockByNumber above.
  describe("vns_getBlockByHash", function() {
    it("should return block given the block hash", function(done) {
      web3.vns.getBlock(0, true, function(err, blockByNumber) {
        if (err) return done(err);

        web3.vns.getBlock(blockByNumber.hash, true, function(err, blockByHash) {
          if (err) return done(err);
  ...
  describe("vns_getBlockTransactionCountByNumber", function(){
    it("should return the number of transactions given the block number (0 transactions)", function(done) {
      //Block 0 should have 0 transactions as per test vns_getBlockByNumber
      web3.vns.getBlock(0, true, function(err, block) {
        if (err) return done(err);
        web3.vns.getBlockTransactionCount(0, function(err, blockTransactionCount) {
    ...
    it("should return the number of transactions given the block number (1 transaction)", function(done) {
      ...
      web3.vns.sendTransaction({
        ...
        web3.vns.getBlock("latest", true, function(err, block) {
          if (err) return done(err);
          web3.vns.getBlockTransactionCount(block.number , function(err, blockTransactionCount) {
            if (err) return done(err);
    ...
    it("should return 0 transactions when the block doesn't exist", function(done) {
      web3.vns.getBlockTransactionCount(1000000, function(err, blockTransactionCount) {
        if (err) return done(err);
        assert.equal(0, blockTransactionCount,  "Block transaction count should be 0.");
        done();
      });
  ...
  // Dependent upon validity of vns_getBlockTransactionCountByNumber
  describe("vns_getBlockTransactionCountByHash", function(){
    it("should return the number of transactions given the hash", function(done) {
      web3.vns.getBlock(0, true, function(err, blockByNumber) {
        if (err) return done(err);
        web3.vns.getBlockTransactionCount(blockByNumber.number, true, function(err, txCountByHash) {
          if (err) return done(err);
            web3.vns.getBlockTransactionCount(blockByNumber.hash , function(err, txCountByNumber) {
              if (err) return done(err);
  ...
  describe("vns_sign", function() {
    ...
    before(function( done ){
      ...
      signingWeb3.vns.getAccounts(function(err, accs) {
        if (err) return done(err);
        accounts = accs.map(function(val) {
          return val.toLowerCase();
        });
    ...
    it("should produce a signature whose signer can be recovered", function() {
      ...
      var msgHash = utils.hashPersonalMessage(msg);
  	  ...
  	  return signingWeb3.vns.sign(utils.bufferToHex(msg), accounts[0]).then(sgn => {
    ...
    it("should work if ecsign produces 'r' or 's' components that start with 0", function() {
      ...
      return signingWeb3.vns.sign(msgHex, accounts[0]).then(sgn => {
        sgn = utils.stripHexPrefix(sgn);
        var r = new Buffer(sgn.slice(0, 64), 'hex');
        var s = new Buffer(sgn.slice(64, 128), 'hex');
        var v = parseInt(sgn.slice(128, 130), 16) + 27;
  ...
  describe('vns_sendRawTransaction', () => {
    ...
    it("should fail with bad nonce (too low)", function(done) {
      ...
      web3.vns.sendSignedTransaction(transaction.serialize(), function(err, result) {
  ...
  describe("contract scenario", function() {
    ...
    it("should add a contract to the network (vns_sendTransaction)", function(done) {
      web3.vns.sendTransaction({
        from: accounts[0],
        data: contract.binary,
        gas: 3141592,
        ...
        web3.vns.getTransactionReceipt(hash, function(err, receipt) {
          if (err) return done(err);
    ...
    it("should verify the transaction immediately (vns_getTransactionReceipt)", function(done) {
      web3.vns.getTransactionReceipt(initialTransaction, function(err, receipt) {
        if (err) return done(err);
    ...
    it("should return null if asked for a receipt for a nonexistent transaction (vns_getTransactionReceipt)", function(done) {
      web3.vns.getTransactionReceipt("0xdeadbeef", function(err, receipt) {
        if (err) return done(err);
    ...
    it("should verify the code at the address matches the runtimeBinary (vns_getCode)", function(done) {
      web3.vns.getCode(contractAddress, function(err, result) {
        if (err) return done(err);
    ...
    it("should have balance of 1 (vns_getBalance)", function(done) {
      web3.vns.getBalance(contractAddress, function(err, result) {
        if (err) return done(err);
    ...
    it("should be able to read data via a call (vns_call)", function(done) {
      ...
      web3.vns.getBlockNumber(function(err, result) {
        if (err) return done(err);

        starting_block_number = result;

        web3.vns.call(call_data, function(err, result) {
          if (err) return done(err);
          ...
          web3.vns.getBlockNumber(function(err, result) {
            ...
            assert.equal(result, starting_block_number, "vns_call increased block count when it shouldn't have");
            done();
          });
        });
      });
    ...
    it("should get back a runtime error on a bad call (vns_call)", function(done) {
      var call_data = _.cloneDeep(contract.call_data);
      call_data.to = contractAddress;
      call_data.from = accounts[0];

      // TODO: Removing this callback hell would be nice.
      web3.vns.estimateGas(call_data, function (err, result) {
        if (err) return done(err);
    ...
    it("should be able to make a call from an address not in the accounts list (vns_call)", function(done) {
    ...
    it("should be able to make a call when no address is listed (vns_call)", function(done) {
      var call_data = contract.call_data;
      call_data.to = contractAddress;
    ...
    it("should represent the block number correctly in the Oracle contract (oracle.blockhash0)", function(done){
      ...
      web3.vns.personal.unlockAccount(accounts[0], "password", function(err, result) {
        var contract = new web3.vns.Contract(JSON.parse(oracleOutput.interface));
        contract.deploy({
          ...
          if (!oracle._requestManager.provider) {
            oracle._requestManager.setProvider(web3.vns._provider);
          }
          web3.vns.getBlock(0, true, function(err, block){
            if (err) return done(err)
    ...
    it("should be able to estimate gas of a transaction (vns_estimateGas)", function(done){
      ...
      web3.vns.getBlockNumber(function(err, result) {
        ...
        web3.vns.estimateGas(tx_data, function(err, result) {
          if (err) return done(err);
          ...
          web3.vns.getBlockNumber(function(err, result) {
            ...
            assert.equal(result, starting_block_number, "vns_estimateGas increased block count when it shouldn't have");
            done();
          });
        });
      });
    ...
    it("should be able to estimate gas from an account not within the accounts list (vns_estimateGas)", function(done){
      var tx_data = contract.transaction_data;
      tx_data.to = contractAddress;
    ...
    it("should be able to estimate gas when no account is listed (vns_estimateGas)", function(done){
      var tx_data = contract.transaction_data;
      tx_data.to = contractAddress;
      delete tx_data.from;
    ...
    it("should be able to send a state changing transaction (vns_sendTransaction)", function(done) {
      var tx_data = contract.transaction_data;
      tx_data.to = contractAddress;
      tx_data.from = accounts[0];
      ...
      web3.vns.sendTransaction(tx_data, function(err, tx) {
        if (err) return done(err);
        ...
        web3.vns.getTransactionReceipt(tx, function(err, receipt) {
          if (err) return done(err);
    ...
    // describe block.
    it("should read data via a call at a specified blockNumber (vns_call)", function(done){
      var startingBlockNumber = null;
      var call_data = contract.call_data;

      web3.vns.getBlockNumber().then(function(result){

        startingBlockNumber = result;
        return web3.vns.call(call_data)

      }).then(function(result){

        assert.equal(to.number(result), 25, "value retrieved from latest block should be 25");
        return web3.vns.call(call_data, contractCreationBlockNumber)

      }).then(function(result){

        assert.equal(to.number(result), 5, "value retrieved from contract creation block should be 5");
        return web3.vns.getBlockNumber()

      }).then(function(result){

        assert.equal(result, startingBlockNumber, "vns_call w/defaultBlock increased block count");
        return web3.vns.call(call_data);
    ...
    it('should read data via a call when specified blockNumber is "earliest" (vns_call)', function(done) {
      var call_data = contract.call_data;

      web3.vns.call(call_data, "earliest").then(function(result){
        assert.equal(to.number(result), 0, "value retrieved from earliest block should be zero");
        done();
      })
    });

    it('should read data via a call when specified blockNumber is "pending" (vns_call)', function(done){
      var call_data = contract.call_data;

      web3.vns.call(call_data, "pending").then(function(result){
        assert.equal(to.number(result), 25, "value retrieved from pending block should be 25");
        done();
      });
    });

    it("should error when reading data via a call at a non-existent blockNumber (vns_call)", function(done){
      ...
      web3.vns.getBlockNumber().then(function(result){
        ...
        return web3.vns.call(call_data, nonExistentBlock);
    ...
    it("should only be able to send an unsigned state changing transaction from an address within the accounts list (vns_sendTransaction)", function(done) {
      ...
      web3.vns.sendTransaction(tx_data, function(err, result) {
        if (err) {
    ...
    it("should get the data from storage (vns_getStorageAt) with padded hex", function(done) {
      web3.vns.getStorageAt(contractAddress, contract.position_of_value, function(err, result) {
        assert.equal(to.number(result), 25);
        done();
      });
    });

    it("should get the data from storage (vns_getStorageAt) with unpadded hex", function(done) {
      web3.vns.getStorageAt(contractAddress, '0x0', function(err, result) {
        assert.equal(to.number(result), 25);
        done();
      });
    });

    it("should get the data from storage (vns_getStorageAt) with number", function(done) {
      web3.vns.getStorageAt(contractAddress, 0, function(err, result) {
        assert.equal(to.number(result), 25);
        done();
      });
    });
  ...
  describe("contract scenario (raw tx)", function() {
    ...
    it("should add a contract to the network (vns_sendRawTransaction)", function(done) {
      web3.vns.sendSignedTransaction(rawTx, function(err, result) {
        if (err) return done(err);
    ...
    it("should verify the transaction immediately (vns_getTransactionByHash)", function(done) {
      web3.vns.getTransaction(initialTransaction, function(err, result) {
        if (err) return done(err);
    ...
    it("should return null if transaction doesn't exist (vns_getTransactionByHash)", function(done) {
      web3.vns.getTransaction("0x401b8ebb563ec9425b052aba8896cb74e07635563111b5a0663289d1baa8eb12", function(err, result) {
        if (err) return done(err);
    ...
    it("should verify there's code at the address (vns_getCode)", function(done) {
      web3.vns.getCode(contractAddress, function(err, result) {
        if (err) return done(err);
    ...
    it("should be able to get the transaction from the block (vns_getTransactionByBlockHashAndIndex)", function(done) {
      web3.vns.getTransactionFromBlock(blockHash, 0, function(err, result) {
        if (err) return done(err);
    ...
    it("should return null if block doesn't exist (vns_getTransactionByBlockHashAndIndex)", function(done) {
      var badBlockHash = "0xaaaaaaeb03ec5e3c000d150df2c9e7ffc31e728d12aaaedc5f6cccaca5aaaaaa";
      web3.vns.getTransactionFromBlock(badBlockHash, 0, function(err, result) {
        if (err) return done(err);
    ...
    it("should be able to get the transaction from the block (vns_getTransactionByBlockNumberAndIndex)", function(done) {
      web3.vns.getTransactionFromBlock(blockNumber, 0, function(err, result) {
        if (err) return done(err);
    ...
    it("should throw error for transactions that don't exist in block (vns_getTransactionByBlockNumberAndIndex)", function(done) {
      web3.vns.getTransactionFromBlock(blockNumber, 3, function(err, result) {
  ...
  describe("vns_getTransactionCount", function() {
    it("should return 0 for non-existent account", function(done) {
      web3.vns.getTransactionCount("0x1234567890123456789012345678901234567890", function(err, result) {
        if (err) return done(err);
  ...
  // describe("vns_compileSolidity (not supported)", function() {
  //   this.timeout(5000);
  //   it("correctly compiles solidity code", function(done) {
  //     web3.vns.compile.solidity(source, function(err, result) {
  //       assert(err != null)
  //       assert(err.message.indexOf("Method vns_compileSolidity not supported") >= 0);
  //       done();
  //     });
  //   });
  // });
  ...
  describe("miner_stop", function(){
    it("should stop mining", function(done){
      web3.currentProvider.send({
        ...
        // we don't use web3.vns.sendTransaction here because it gets huffy waiting for a receipt,
  ...
  describe("net_version", function() {
    it("should return a version very close to the current time", function(done) {
      web3.vns.net.getId(function(err, result) {
        if (err) return done(err);
  ...
  describe("personal_newAccount", function() {
    it("should return the new address", function(done) {
      web3.vns.personal.newAccount("password", function(err, result) {
        if (err) return done(err);
  ...
  describe("personal_listAccounts", function() {
    it("should return more than 0 accounts", function(done) {
      web3.vns.personal.getAccounts(function(err, result) {
        if (err) return done(err);
  ...
  describe("personal_unlockAccount", function() {
    it("should unlock account", function(done) {
      web3.vns.personal.unlockAccount(personalAccount, "password", function(err, result) {
        if (err) return done(err);
  ...
  describe("personal_lockAccount", function() {
    it("should lock account", function(done) {
      web3.vns.personal.lockAccount(personalAccount, function(err, result) {
        if (err) return done(err)
        assert.equal(result, true);
        done()
      });
  ...
  /*describe("personal_sendTransaction", function() {
    it("should send transaction", function(done) {
      web3.vns.sendTransaction({value: web3.utils.toWei('5', 'ether'), from: accounts[0], to: personalAccount }, function(err, result){
        if (err) return done(err)

        web3.vns.personal.sendTransaction({
