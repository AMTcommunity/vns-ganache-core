...
describe("Mining", function() {
  ...
  function getBlockNumber() {
    return new Promise(function(accept, reject) {
      web3.vns.getBlockNumber(function(err, number) {
  ...
  function getReceipt(tx) {
    return new Promise(function(accept, reject) {
      web3.vns.getTransactionReceipt(tx, function(err, result) {
        if (err) return reject(err);
        accept(result);
      });
    });
  ...
  function getCode(address) {
    return new Promise(function(accept, reject) {
      web3.vns.getCode(address, function(err, result) {
        if (err) return reject(err);
        accept(result);
      });
    });
  ...
  it("should error via instamining when queued transaction throws a runtime errors", function(done) {
    ...
    startMining().then(function() {
      ...
      throw new Error("Execution should never get here as we expected `vns_sendTransaction` to throw an error")
    }).catch(function(err) {
  ...
  it("should return the correct value for vns_mining when miner started and stopped", function() {
    return stopMining().then(function() {
      return checkMining();
